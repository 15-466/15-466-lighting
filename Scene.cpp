...
void Scene::load(std::string const &filename,
	...
	std::vector< CameraEntry > cameras_entries;
	read_chunk(file, "cam0", &cameras_entries);
	...
	std::vector< LightEntry > lights_entries;
	read_chunk(file, "lmp0", &lights_entries);
	...
	for (auto const &c : cameras_entries) {
	...
	for (auto const &l : lights_entries) {
